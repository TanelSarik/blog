    ...
    <div class="card bg-base-300">
        <div class="card-body">
            ...
            <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <fieldset class="fieldset">
                    <legend class="fieldset-legend">Images</legend><input  name="images[]" multiple type="file" accept="image/*" class="file-input w-full @error('images.*') file-input-error @enderror" /><input name="images[]" multiple type="file" class="file-input w-full @error('images.*') file-input-error @enderror" />
                    @error('images.*')
                        <p class="label text-error">{{ $message }}</p>
                    @enderror
                </fieldset>
